...
template <typename Event, typename F, typename ...Fs>
inline auto match_impl(Event && event, F && f, Fs &&... fs)
{
#define CASE_MATCH_CHECK(T)                                                                                \
    case std::decay_t<T>::value:                                                                           \
        if constexpr (std::is_invocable_v<F, T>) {                                                         \
            return std::forward<F>(f)(T{std::forward<Event>(event).handler().*(std::decay_t<T>::member)}); \
        } else {                                                                                           \
            static_assert(sizeof...(Fs) > 0, "non-exhaustive list of functions provided");                 \
            return match_impl(std::forward<Event>(event), std::forward<Fs>(fs)...);                        \
        }
...
/* { */
template <typename Event, typename... Fs>
inline auto match(Event && event, Fs &&... fs)
{
    static_assert(
            std::is_same_v<std::decay_t<Event>, SDLpp::event>,
            "`match's first argument must be an event"
        );
    return detail::match_impl(std::forward<Event>(event), std::forward<Fs>(fs)...);
}
/* } // namespace experimental */
