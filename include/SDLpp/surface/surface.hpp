...
{
    if ( rect.has_value() ) {
        SDL_FillRect( handler(), std::addressof(*rect), color);
    }
    else {
        SDL_FillRect( handler(), nullptr, color ); //SDL_FillRect should not own the pointer
    }
...
//inline auto surface::pixel_format() const -> SDL_PixelFormat *
//{
    //if ( ! _handler ) {
        //return nullptr;
    //}
